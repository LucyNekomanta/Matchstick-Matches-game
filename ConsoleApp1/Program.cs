...
namespace _7_Aufgabe_7__NIMM_DREI__Dennis_Nicolai {
	class Program {
		...
		static void CoinTossFunction() {
			Console.WriteLine(
			    "Now we toss a coin to decide who goes first, choose: Heads(1), or Tails(2) ?{0}" +
		...
		// checks if input is a number that is bigger than 0 or in range
		{
			...
			if (!enableLimitlessNumCheck) {
				while (!numCheck) {
					...
					if (int.TryParse(input, out numCheckOutput) &&
					    numCheckOutput > 0 &&
					    numCheckOutput <= dynamicNumCheck) {
						numCheck = true;
					} else {
						Console.WriteLine(
						    "You have to write a number that is in the range of 1 and " +
						    dynamicNumCheck);
					}
				}
			} else {
				while (!numCheck) {
					...
					if (int.TryParse(input, out numCheckOutput) &&
						...
						Console.WriteLine(
						    "You have to write a number that is bigger than zero.");
					}
				}
			}
		}
		...
		// human vs human
		{
			...
			while (MatchsticksNumber > 0) {
...

				{
					ShowMatchsticksFunction();
					Console.Write(
					    "Player2, choose a number from 1 to 3 to take: ");
					NumCheckFunction();
					...
					{
						Console.WriteLine(
						    "Player2 lost, Player1 wins the game");
					}
				}
			}
		}
		static void PlayerSelect() {
			Console.WriteLine(
			    "Do you want to play against an AI player(1) or against a human player(2)(who has to sit next to you)?{0}" +
...

		{
			...
			{
				ShowMatchsticksFunction();
				// Console.WriteLine("There are {0} matches, choose a number in the
				// range of 1 to 3 to take.", MatchsticksNumber);
